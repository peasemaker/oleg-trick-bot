...
export default class SemiRandomBot extends ChessGame {
  getNextMove(): number {
    ...
    console.log('safeMoves', safeMoves.map(m => ChessGame.numericToUci(m)).join('; '));
    ...
    if (checkmates.length) {
      ...
      pickedMove = safeMoves[Math.floor(Math.random() * safeMoves.length)];
    }
