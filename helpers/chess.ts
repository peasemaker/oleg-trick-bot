...
export default class ChessGame {
  ...
  static literalToSquare(literal: string): number {
    return sq120(files.indexOf(literal[0]) + ranks.indexOf(literal[1])) * 8;
  }
    ...
    for (let i = 0; i < 64; i++) {
      if (this.turn === Side.WHITE) {
        if (this.board[sq120(i)] === Pieces.wP) {
          if (ChessGame.isBlackPiece(this.board[sq120(i) - 9])) {
            moves.push(ChessGame.squareToLiteral(sq120(i)) + ChessGame.squareToLiteral(sq120(i) - 9));
          }
          if (ChessGame.isBlackPiece(this.board[sq120(i) - 11])) {
            moves.push(ChessGame.squareToLiteral(sq120(i)) + ChessGame.squareToLiteral(sq120(i) - 11));
          }
          if (this.board[sq120(i) - 10] === Squares.EMPTY) {
            moves.push(ChessGame.squareToLiteral(sq120(i)) + ChessGame.squareToLiteral(sq120(i) - 10));
            if (Math.floor(i / 8) === Ranks.RANK_2) {
              moves.push(ChessGame.squareToLiteral(sq120(i)) + ChessGame.squareToLiteral(sq120(i) - 20));
            }
          }
        }
      } else {
        if (this.board[sq120(i)] === Pieces.bP) {
          if (ChessGame.isWhitePiece(this.board[sq120(i) + 9])) {
            moves.push(ChessGame.squareToLiteral(sq120(i)) + ChessGame.squareToLiteral(sq120(i) + 9));
          }
          if (ChessGame.isWhitePiece(this.board[sq120(i) + 11])) {
            moves.push(ChessGame.squareToLiteral(sq120(i)) + ChessGame.squareToLiteral(sq120(i) + 11));
          }
          if (this.board[sq120(i) + 10] === Squares.EMPTY) {
            moves.push(ChessGame.squareToLiteral(sq120(i)) + ChessGame.squareToLiteral(sq120(i) + 10));
            if (Math.floor(i / 8) === Ranks.RANK_7) {
              moves.push(ChessGame.squareToLiteral(sq120(i)) + ChessGame.squareToLiteral(sq120(i) + 20));
            }
          }
        }
